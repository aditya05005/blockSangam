...
def test_scenario_comparison_is_derived_from_actual_assignments():
    ...
    assert result["base"]["summary"]["tasks_scheduled"] == 23
    assert result["scenario_result"]["summary"]["tasks_scheduled"] == 23
...
def test_schedule_can_be_exported_as_a_real_pdf():
    client = TestClient(create_app("sqlite://"))
    schedule = client.post("/api/schedule", json={"scenario": "base", "max_solve_time": 1})
    assert schedule.status_code == 200

    exported = client.post("/api/reports/plan.pdf", json=schedule.json())
    assert exported.status_code == 200
    assert exported.headers["content-type"] == "application/pdf"
    assert exported.content.startswith(b"%PDF-")
    assert len(exported.content) > 5_000


def test_department_feedback_round_trip_with_coa():
    client = TestClient(create_app("sqlite://"))
    created = client.post("/api/feedback", json={
        "sender_role": "ENGINEERING", "recipient_role": "COA",
        "department": "ENGINEERING", "task_id": "ENG-006",
        "message": "Move this package to the later possession window.",
    })
    assert created.status_code == 201
    feedback_id = created.json()["feedback_id"]

    reply = client.post("/api/feedback", json={
        "sender_role": "COA", "recipient_role": "ENGINEERING",
        "department": "ENGINEERING", "task_id": "ENG-006",
        "parent_id": feedback_id, "message": "Reviewed. A revised plan will be issued.",
    })
    assert reply.status_code == 201
    assert client.patch(f"/api/feedback/{feedback_id}", json={"status": "UNDER_REVIEW"}).status_code == 200
    items = client.get("/api/feedback", params={"role": "ENGINEERING"}).json()["items"]
    assert len(items) == 2
    assert {item["sender_role"] for item in items} == {"COA", "ENGINEERING"}


def test_role_logins():
    client = TestClient(create_app("sqlite:///:memory:"))
    for username, role in (("coa", "COA"), ("engineering", "ENGINEERING"), ("snt", "SNT"), ("trd", "TRD")):
        response = client.post("/api/auth/login", json={"username": username, "password": "blocksangam"})
        assert response.status_code == 200
        assert response.json()["role"] == role
    rejected = client.post("/api/auth/login", json={"username": "coa", "password": "incorrect"})
