...
def _mumbai_section(value: str) -> str:
    """Accept legacy prototype corridor codes while exposing Mumbai names."""
    return {"A-B": "Thane-Kurla", "B-C": "Kurla-Chembur", "C-D": "Chembur-Mankhurd"}.get(value, value)
...
def execute_custom_scenario(changes: dict[str, Any], base_dir: str | Path, *, max_solve_time_seconds: float = 10.0) -> ScenarioExecution:
    ...
    if closure:
        section, line = _mumbai_section(closure["section"]), Line(closure["line"])
    ...
    if task_input:
        ...
        section, line = _mumbai_section(task_input["section"]), Line(task_input["line"])
        if not any(slot.section == section and slot.line == line for slot in base.corridor_slots):
            raise ValueError(f"Unknown corridor: {section} {line.value}")
        task = Task(
